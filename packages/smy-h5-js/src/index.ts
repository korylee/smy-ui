...
const staticRoot =
  ...
  (~['https://www.smyfinancial.com', 'https://wwwjs.smyfinancial.com', 'https://wwwhb.smyfinancial.com'].indexOf(
    APIHost
  )
...
const SMYJS = {
  ...
  env: {
    ...
    phoneModel:
      ua
        ...
        .split('/')[1]
        .replace(/_/g, ' ') ?? '', // 手机型号
    phoneMarker: /(iPhone|iPad|iPod|iOS)/i.test(ua) ? 'apple' : ua.match(/phonemarker\/\S*/gi)?.[0].split('/')[1] ?? '', // 手机品牌
  ...
  checkSFZ: function (sfz: string) {
    //检测是否合法身份证
    return /^[1-9][0-9]{5}(19[0-9]{2}|200[0-9]|2010)(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])[0-9]{3}[0-9xX]$/.test(sfz)
  },
  checkChineseName,
  checkEmail,
  compareVersion,
  ...
  openNewWebviewPage(url, needLogin) {
    ...
    if (
      ...
      (SMYJS.env.isIOS || (SMYJS.env.isAndroid && SMYJS.compareVersion(SMYJS.env.isShengbei, '7.18.0')))
    ) {
