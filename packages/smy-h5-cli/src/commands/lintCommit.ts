...
const isVersion = (msg: string) => msg.startsWith('v') && semver.valid(msg.slice(1))

export function lintCommit(gitParams: string) {
  ...
  if (!isVersion(message) && !COMMIT_MESSAGE_RE.test(message)) {
    ...
    logger.warning(`\
The rules for commit message are as follows
