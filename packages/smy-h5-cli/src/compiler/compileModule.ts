...
import { ES_DIR, EXAMPLE_DIR_NAME, LIB_DIR, SRC_DIR, STYLE_DIR_NAME } from "../shared/constant";
import { resolve } from "path";
import { getPublicDirs, isDir, isDTS, isScript, isSFC } from "../shared/fs-utils";
...
export async function compileDir(dir: string) {
  ...
  await Promise.all(
    dirs.map((filename) => {
      ...
      [EXAMPLE_DIR_NAME].includes(filename) && removeSync(file);
